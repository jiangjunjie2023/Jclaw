...
# 全局配置
DEFAULT_BASE_URL = "https://gateway.test.xdf.cn/qiwei-open"
ENDPOINT = "/msg/staff/chat"
...
def compute_sign(app_key: str, param_str: str) -> tuple[str, str]:
    """
    计算签名（返回签名+原始哈希16进制）
    :param app_key: 密钥（对应Java的appKey）
...
def _request(
    ...
    # 计算签名
    sign, raw_hash_hex = compute_sign(app_key, param_str)
...
def main() -> None:
    """主函数"""
    # 解析命令行参数
    parser = argparse.ArgumentParser(description="企业微信消息查询 OpenAPI 客户端")
    parser.add_argument(
        "--payload-file",
        type=str,
        required=True,  # 强制要求用户指定，无默认值
        help="JSON请求体文件路径（必填，例如：scripts/examples/query.json）"
    )
    parser.add_argument(
        "--base-url",
        type=str,
    ...
    # 获取鉴权信息
    app_id = _get_env("QIWEI_OPEN_APP_ID")
    app_key = _get_env("QIWEI_OPEN_APP_KEY")
    corp_id = _get_env("QIWEI_OPEN_CORP_ID")

    # 加载并序列化请求体
    param_str = load_payload_file(args.payload_file)

    # 发送请求
    result = _request(args.base_url, ENDPOINT, app_id, app_key, param_str)
